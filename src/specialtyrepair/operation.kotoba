(ns specialtyrepair.operation
  "OperationActor -- one repair operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (RepairOps-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Repair Shop Governor (:govern) and the rollout
  ...
  One graph run = one repair operation (intake -> advise -> govern ->
  ...
  decision to a human operator (repair technician/shop owner). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:repair/complete`/`:item/return` ALWAYS reach this node
  when the governor is clean -- see `specialtyrepair.phase`."
...
(defn build
  ...
  `specialtyrepair.store/Store`).
  opts:
    :advisor      -- a `specialtyrepair.repairopsllm/Advisor` (default: mock-advisor)
